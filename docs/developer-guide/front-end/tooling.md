...
Updating and running the tooling in wagtail-kit is a bit fiddly, as if you create a `node_modules` folder in `/wagtail-kit/drnalpha` the watcher script will throw an error.
...
The easiest way to install new packages is to run `npm install --save-dev package_name` in `wagtail-kit/built_image/ck_repo_name`, but then copy the resultant changes to `package.json` and `package-lock.json` over to `/wagtail-kit/drnalpha` in order to save and commit them. Note that when you do this you need to edit the copied files and change all instances of `ck_repo_name` to `drnalpha`. Also make sure you turn off the watcher.py script before making changes in `built_image`.

Changes to the config files can be made directly in `/wagtail-kit/drnalpha`.
