"""
Django settings for drnalpha project.
"""
import os
import sys
...
INSTALLED_APPS = [
    ...
    "drnalpha.images",
    "drnalpha.navigation",
    "drnalpha.regulations",
    "drnalpha.search",
    "drnalpha.sic_codes",
    "drnalpha.standardpages",
    "drnalpha.tracker",
    "drnalpha.users",
    "drnalpha.utils",
    ...
    "pattern_library",
    "drnalpha.project_styleguide.apps.ProjectStyleguideConfig",
    "wagtailaccessibility",
    "crispy_forms",
]

if (
    env.get("SCOUT_MONITOR", "false").strip().lower() == "true"
    and env.get("SCOUT_KEY", "").strip()
):
...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "drnalpha.utils.context_processors.global_vars",
            ],
...
WSGI_APPLICATION = "drnalpha.wsgi.application"
...
DATABASES = {
    "default": dj_database_url.config(conn_max_age=600, default="postgres:///drnalpha")
}
...
if "SENTRY_DSN" in env:
    ...
    # dokku config:set drnalpha SENTRY_ENVIRONMENT=staging
...
if env.get("BASIC_AUTH_ENABLED", "false").lower().strip() == "true":
    ...
    # "llamasavers.production.example.com".
...
COMPANIES_HOUSE_API_KEY = env.get("COMPANIES_HOUSE_API_KEY", "")

NOTIFY_KEY = env.get("NOTIFY_KEY")
