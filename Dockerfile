...
COPY ./drnalpha/static_src/ ./drnalpha/static_src/
...
COPY --chown=drnalpha pyproject.toml poetry.lock ./
RUN if [ "$BUILD_ENV" = "dev" ]; then poetry install --extras gunicorn; else poetry install --no-dev --extras gunicorn; fi

COPY --chown=drnalpha --from=frontend ./drnalpha/static_compiled ./drnalpha/static_compiled
...
CMD gunicorn drnalpha.wsgi:application
